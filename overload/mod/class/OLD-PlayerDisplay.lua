...
function _M:display()
	...
	if player:knowTalent(player.T_LOYALTY_POOL) then
		self:mouseTooltip(self.TOOLTIP_LOYALTY, self:makeTextureBar("#7fffd4#Loyalty:", nil, player:getLoyalty(), player:getMaxLoyalty(), player.loyalty_regen, x, h, 255, 255, 255,
			{r=colors.YELLOW.r / 2, g=colors.YELLOW.g / 2, b=colors.YELLOW.b / 2},
			{r=colors.YELLOW.r / 5, g=colors.YELLOW.g / 5, b=colors.YELLOW.b / 5}
		)) h = h + self.font_h
	end
